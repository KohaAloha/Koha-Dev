...
<xsl:stylesheet version="1.0"
 ...
 <span class="label">Contenu incomplet :</span>
 </xsl:when>
 ...
 <span class="label">Formé de la réunion de ... and ...</span>
 </xsl:when>
 ...
 <xsl:when test="@ind2=8">
 <span class="label">Retourner à</span>
 </xsl:when>
 ...
 <a><xsl:attribute name="href">http://<xsl:value-of select="$OPACBaseURL"/>/cgi-bin/koha/opac-detail.pl?biblionumber=<xsl:value-of select="marc:datafield[@tag=999]/marc:subfield[@code='c']"/></xsl:attribute><xsl:attribute name="target">_blank</xsl:attribute>Ouvrir dans une nouvelle fenêtre</a>. </span>
 </xsl:if>
 ...
 <xsl:when test="@tag&lt;700">Auteur(s)&nbsp;: </xsl:when>
 <xsl:otherwise>Autres auteurs&nbsp;:</xsl:otherwise>
 </xsl:choose>
 <a>
 <xsl:choose>
 <xsl:when test="marc:subfield[@code=9] and $UseAuthoritiesForTracings='1'">
