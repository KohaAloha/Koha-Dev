...
<xsl:stylesheet version="1.0"
 ...
 <xsl:choose>
 <xsl:when test="$leader19='a'">Groupe</xsl:when>
 <xsl:when test="$leader6='a'">
 <xsl:choose>
 <xsl:when test="$leader7='c' or $leader7='d' or $leader7='m'">Livre</xsl:when>
 <xsl:when test="$leader7='i' or $leader7='s'">
 <xsl:choose>
 <xsl:when test="substring($controlField008,22,1)!='m'">Ressource continue</xsl:when>
 ...
 <h5 class="author">par <xsl:call-template name="showAuthor">
 ...
 <span class="results_summary series"><span class="label">Collections&nbsp;: </span>
 <!-- 440 -->
 <xsl:for-each select="marc:datafield[@tag=440]">
 <a><xsl:attribute name="href">/cgi-bin/koha/opac-search.pl?q=ti,phr:"<xsl:value-of select="marc:subfield[@code='a']"/>"</xsl:attribute>
 ...
 <xsl:text>Voir les volumes</xsl:text>
 </a>
 </span>
 </xsl:if>
 ...
 <xsl:with-param name="label">Titres uniformes : </xsl:with-param>
 </xsl:call-template>
 </xsl:if>

 <xsl:if test="marc:datafield[@tag=130]|marc:datafield[@tag=240]|marc:datafield[@tag=730][@ind2!=2]">
 <span class="results_summary uniform_titles"><span class="label">Titres uniformes : </span>
 ...
 <span class="results_summary subjects"><span class="label">Sujet(s)&nbsp;: </span>
...
<!-- Image processing code added here, takes precedence over text links including y3z text -->
 ...
 <span class="label">Contenus incomplets :</span>
 </xsl:when>
 <xsl:when test="@ind1=2">
 <span class="label">Contenu partiel :</span>
 </xsl:when>
 <xsl:otherwise>
 ...
 <xsl:choose>
 <xsl:when test="@ind1=0"><xsl:text>Sujet: </xsl:text></xsl:when>
 <xsl:when test="@ind1=1"><xsl:text>Commentaires: </xsl:text></xsl:when>
 <xsl:when test="@ind1=2"><xsl:text>Contenu et portée : </xsl:text></xsl:when>
 <xsl:when test="@ind1=3"><xsl:text>Résumé : </xsl:text></xsl:when>
 <xsl:when test="@ind1=4"><xsl:text>Conseil de contenu&nbsp;: </xsl:text></xsl:when>
