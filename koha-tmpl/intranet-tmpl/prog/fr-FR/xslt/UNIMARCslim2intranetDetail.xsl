...
<xsl:template match="marc:record">
 ...
 <xsl:with-param name="label">Auteur principale collectivité</xsl:with-param>
 </xsl:call-template>
 ...
 <xsl:when test="@code='c'">d'oeuvre original, </xsl:when>
 <xsl:when test="@code='d'">de résumé, </xsl:when>
 <xsl:when test="@code='e'">de page de contenu, </xsl:when>
 <xsl:when test="@code='f'">de page-titre, </xsl:when>
 ...
 <xsl:when test="@code='j'">de sous-titres </xsl:when>n> </xsl:choose>
...
<xsl:if test="marc:controlfield[@tag=009]">
 ...
 Cliquez ici pour l'accès en ligne</xsl:when>
 </xsl:choose>
 </a>
 <xsl:choose>
 ...
 <strong>Formé de la réunion de ... and ...</strong>
 </xsl:when>
